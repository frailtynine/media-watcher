...
class Prompt(Base):
    ...
    role: Mapped[str] = mapped_column(
        Text(3000), nullable=False, default=Prompts.ROLE
    )
    crypto_role: Mapped[str] = mapped_column(
        Text(3000), nullable=False, default=Prompts.ROLE_CRYPTO
    )
    suggest_post: Mapped[str] = mapped_column(
        Text(3000), nullable=False, default=Prompts.SUGGEST_POST
    )
